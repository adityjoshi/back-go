...
const WardenComplaints = () => {
  ...
  const handleApproval = async (id) => {
    try {
      const response = await fetch(`http://localhost:2426/complaints/${id}`, {
        method: "PUT",
        headers: GetAuthHeader(),
      });
      const complaintData = await response.json();

      const updatedStatus = !complaintData.is_completed;

      const updateResponse = await fetch(`http://localhost:2426/complaints/${id}`, {
        method: "PUT",
        headers: GetAuthHeader(),
